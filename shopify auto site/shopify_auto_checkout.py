...
class ShopifyGlobalState:
    ...
    async def ban_site_permanently(self, site, reason=""):
    ...
    def mark_site_unavailable(self, site):
        ...
        # Note: Using asyncio.create_task because this legacy method is synchronous
        # but the new ban method is asynchronous.
        import asyncio
        try:
            loop = asyncio.get_event_loop()
            if loop.is_running():
                loop.create_task(self.ban_site_permanently(site, "Response validation failed"))
            else:
                asyncio.run(self.ban_site_permanently(site, "Response validation failed"))
        except RuntimeError:
            asyncio.run(self.ban_site_permanently(site, "Response validation failed"))
                    ...
                    if 'errors' in completion and completion['errors']:
                        ...
                        if non_pending_errors:
                            ...
                            if any(e in challenge_errors for e in non_pending_errors):
                                ...
                                await GLOBAL_STATE.ban_site_permanently(site_url, f"Dead site response: {error_msg}")
